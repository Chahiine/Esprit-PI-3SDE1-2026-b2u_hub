...
export class ProfileComponent implements OnInit {
  ...
  resetForm(): void {
    ...
    this.bio = u.bio ?? ''; this.skills = u.skills ?? '';
  }
  ...
  saveProfile(): void {
    this.clearMsg();
    this.auth.updateProfile({ firstName: this.firstName, lastName: this.lastName, bio: this.bio, skills: this.skills }).subscribe({
      next: (res) => { this.user.set(res); this.editing = false; this.successMsg = 'Profil mis à jour !'; setTimeout(() => this.successMsg = '', 3000); },
  ...
  savePassword(): void {
    ...
    this.auth.changePassword({ currentPassword: this.currentPassword, newPassword: this.newPassword }).subscribe({
      next: () => { this.successMsg = 'Mot de passe changé !'; this.showPassword = false; this.currentPassword = this.newPassword = this.confirmPassword = ''; setTimeout(() => this.successMsg = '', 3000); },
  ...
  onImageSelected(event: Event): void {
    ...
    this.auth.uploadProfileImage(file).subscribe({
      next: () => { this.user.set(this.auth.user()); this.imageUploading = false; this.successMsg = 'Photo mise à jour !'; setTimeout(() => this.successMsg = '', 3000); },
  ...
  onCvSelected(event: Event): void {
    ...
    this.auth.uploadCv(file).subscribe({
      next: () => { this.user.set(this.auth.user()); this.cvUploading = false; this.successMsg = 'CV uploadé !'; setTimeout(() => this.successMsg = '', 3000); },
